from Crypto.PublicKey import RSA
import base64

# The PEM-formatted RSA key with removed headers and footers
pem_key = """

nh6tZ+tVQCRvFviIEGkaXdEiBN4eTbcjfc5md/u9eA5N21Pzgd/G
"""

def extract_private_key_d(pem_key):
    
    # Decode base64
    der_data = base64.b64decode(pem_key)

    # Import the key using PyCryptodome
    key = RSA.import_key(der_data)

    # Extract and return the private key d as a decimal integer
    return key.d

# Extract private key d
private_key_d = extract_private_key_d(pem_key)

# Print the result
print("Private Key (d):", private_key_d)
